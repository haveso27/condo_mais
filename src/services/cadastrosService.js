...
export const moradoresService = {
  listar(records, { query = '', statuses = [], tower = '', unit = '', relation = '' } = {}) { return records.filter((item) => matchesQuery(item, query) && (!statuses.length || statuses.includes(item.status)) && (!tower || item.tower === tower) && (!unit || item.unit === unit) && (!relation || (relation === 'PROPRIETARIO' ? item.isOwner : item.isResident))) },
  criar(records, payload) { const record = { ...payload, id: nextId('MOR', records), status: 'ATIVO' }; return { records: [record, ...records], record } },
