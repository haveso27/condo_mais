...
function toneFor(status = '') { return /cancel|recus|urgente/i.test(status) ? 'red' : /aguardando|analise|pendente/i.test(status) ? 'violet' : 'green' }
...
const visitorFields = [{ key: 'name', label: 'Nome do visitante', required: true }, { key: 'cpf', label: 'CPF', required: true }, { key: 'phone', label: 'Telefone' }, { key: 'date', label: 'Data prevista', type: 'date', required: true }, { key: 'time', label: 'Horário previsto', type: 'time', required: true }, { key: 'notes', label: 'Observação', type: 'textarea' }]
...
const ticketFields = [{ key: 'title', label: 'Título', required: true }, { key: 'description', label: 'Descrição', type: 'textarea', required: true }, { key: 'category', label: 'Categoria', type: 'select', options: ['Manutenção', 'Convivência', 'Segurança', 'Outros'], required: true }, { key: 'location', label: 'Local', required: true }, { key: 'priority', label: 'Prioridade', type: 'select', options: ['BAIXA', 'NORMAL', 'ALTA', 'URGENTE'], required: true }, { key: 'notes', label: 'Observação', type: 'textarea' }]

const listConfigs = {
  visitantes: { title: 'Visitantes', subtitle: 'Gerencie o acesso dos seus visitantes.', action: 'Autorizar visitante', tabs: [['Todos', []], ['Pendentes', ['PENDENTE']], ['Autorizados', ['AUTORIZADO']], ['Presentes', ['ENTROU']], ['Histórico', ['SAIU', 'RECUSADO']]], fields: visitorFields, icon: UserRound, empty: 'Nenhum visitante encontrado.' },
  ...
  chamados: { title: 'Chamados', subtitle: 'Acompanhe suas solicitações ao condomínio.', action: 'Abrir chamado', tabs: [['Todos', []], ['Abertos', ['ABERTO']], ['Em análise', ['EM_ANALISE']], ['Em atendimento', ['EM_ATENDIMENTO']], ['Resolvidos', ['RESOLVIDO', 'ENCERRADO']]], fields: ticketFields, icon: Wrench, empty: 'Nenhum chamado encontrado.' },
...
export function ResidentListPage({ type }) {
  ...
  function summary(record) {
    if (type === 'visitantes') return [record.date, record.time, statusLabel(record.status)]
  ...
  if (type === 'aprovacoes' && !data.currentResident.isOwner) return <ResidentLayout><section className="resident-page"><ResidentPageHeader title="Aprovações" subtitle="Acesso exclusivo para proprietários." /><EmptyState title="Seu vínculo não possui permissão para aprovar reservas." /></section></ResidentLayout>
  const Icon = config.icon
  return <ResidentLayout><section className="resident-page"><ResidentPageHeader title={config.title} subtitle={config.subtitle} action={config.action} onAction={() => { setForm({}); setError(''); setOpen(true) }} /><div className="mobile-tabs">{config.tabs.map(([label], index) => <button className={tab === index ? 'active' : ''} onClick={() => setTab(index)} key={label}>{label}</button>)}</div><h3 className="section-label">{config.title}</h3><div className="resident-card-list">{records.map((record) => { const info = summary(record); return <button key={record.id} onClick={() => setDetails(record)}><Icon size={21} /><span><strong>{record.name || record.area || record.title}</strong><small>{info[0]}</small><small>{info[1]}</small><StatusBadge tone={toneFor(record.status)}>{info[2]}</StatusBadge></span><ChevronRight size={16} /></button> })}</div>{records.length === 0 && <EmptyState title={config.empty} onClear={tab ? () => setTab(0) : undefined} />}</section>
    <Modal open={open} title={config.action} onClose={() => setOpen(false)} onConfirm={save} confirmLabel={config.action}><DynamicForm fields={config.fields} values={form} onChange={(key, value) => { setForm((current) => ({ ...current, [key]: value })); setError('') }} error={error} />{availability && <p className={availability.available ? 'availability availability--ok' : 'availability availability--error'}>{availability.message}</p>}{type === 'visitantes' && <p className="form-note">Unidade vinculada: {data.currentResident.unit} · {data.currentResident.tower}</p>}</Modal>
